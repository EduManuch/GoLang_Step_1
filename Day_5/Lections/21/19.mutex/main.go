...
// не должны одновременно использовать какой-то общий экземпляр 
// (файл, переменную, бд) для одновременных модификаций.

// 2. Пример RS
// Допустим у нас код
//     x = x + 1
// Пока работает 1 горутина - проблем никаких нет и инкремент стандартный
// Теперь представим что горутин 2
// ПОСКОЛЬКУ ДЛЯ ОБЕИХ ГОРУТИН ДАННЫЙ РЕСУРС БУДЕТ ИСПОЛЬЗОВАТЬСЯ КОНКУРЕТНО, ОЖИДАТЬ ЧТО ПО ИТОГУ РАБОТУ ОБЕИХ ГОРУТИН
...
// * В итоге будет X = 1
// * Первая горутина начинает работать, увеличивает x на единицу, но не успевает присвоить его
...
// Для того, чтобы избежать этой пробелмы использую мьютексы (мьютекс блокирует ресурс до тех пор, пока его не освободит одна из горутин)
